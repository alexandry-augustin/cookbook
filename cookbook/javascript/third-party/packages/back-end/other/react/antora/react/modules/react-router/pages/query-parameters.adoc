...
[,]
----
// A conceptual example of a custom hook
function useQueryParams() {
  const [searchParams, setSearchParams] = useSearchParams(); // using react-router-dom v6 hook
...
const handleFilterChange = (event) => {
  // Call the centralized function to update a single parameter 'category'
